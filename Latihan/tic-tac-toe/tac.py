...
boardHeight = int(input('How high should the board be? (3) ') or 3 )
boardWidth = int(input('How wide should the board be? ('+str(boardHeight)+') ') or boardHeight)
lowest= sorted((boardHeight,boardWidth))[0]
winLen = int(input('How many in a row to win? (' + str(lowest) + ') ') or lowest)
...
overWriteStr = input('Should markers be overwriteable? [y/n] ')
if overWriteStr == 'n':
    overWriteBool = False
else:
    overWriteBool = True
